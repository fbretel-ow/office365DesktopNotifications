(function () {
  ...
  const CHECK_INTERVAL_MILLISECONDS = 10000;
  ...
  const NOTIFIERS = [
    {
      ...
      itemSelector     : 'div._lvv_w:not(._lvv_A)',
      propsToSelectors : {
  ...
  function scrapeForUnreadObjects(itemSelector, propsToSelectors) {
    ...
    return reminderNodes
      .map(x => Object.keys(propsToSelectors)
           .reduce((data, prop) => {
             var item = x.querySelector(propsToSelectors[prop]);
             data[prop] = item && item.textContent;
             return data;
           }, {}))
      .map(value => ({
  ...
  function getUnreadEmailsCount(){
    ...
    return elt ? parseInt(elt.textContent.replace(/[^0-9]/g, ''), 10) : 0;
  }
  ...
  function runNotifiers(oldHashes = {}) {
    return NOTIFIERS.reduce((acc, notifier) => {
      ...
      // for scripting and further desktop integration
      localStorage.setItem(notifier.name, unreadObjects.length);
  ...
  window.ExtO365DN_injected = true;
